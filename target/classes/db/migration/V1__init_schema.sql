...
ALTER TABLE user_preferences
    ADD COLUMN IF NOT EXISTS wind_sensitivity VARCHAR(20) NOT NULL DEFAULT 'MEDIUM',
    ADD COLUMN IF NOT EXISTS rain_sensitivity VARCHAR(20) NOT NULL DEFAULT 'MEDIUM',
    ADD COLUMN IF NOT EXISTS max_layers SMALLINT NOT NULL DEFAULT 3,
    ADD COLUMN IF NOT EXISTS prefers_headwear BOOLEAN NOT NULL DEFAULT FALSE,
    ADD COLUMN IF NOT EXISTS prefers_waterproof BOOLEAN NOT NULL DEFAULT FALSE,
    ADD COLUMN IF NOT EXISTS activity_level VARCHAR(20) NOT NULL DEFAULT 'MEDIUM';
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_user_preferences_max_layers'
    ) THEN
        ALTER TABLE user_preferences
            ADD CONSTRAINT chk_user_preferences_max_layers
            CHECK (max_layers BETWEEN 1 AND 5);
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_user_feedback_recommendation_history_id ON user_feedback(recommendation_history_id);
